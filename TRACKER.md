# ✅ Master Tracker

> 📱 **Best experience: open [`index.html`](index.html) on your phone** — it's an interactive
> tracker that saves your progress. Live URL (after GitHub Pages is on):
> **https://tsharrer.github.io/the-party-porch/**
>
> This file is the plain-text mirror.

Legend: 💰 spend · 📹 content · 📣 marketing · 🎯 milestone

## ✅ Brand (DONE)
- [x] Secure name, domain (yourpartyporch.com) + all social handles

## July 2026 — Foundation
- [ ] Create business Gmail + password manager
- [ ] Set up free Google Business Profile (service-area)
- [ ] File DBA / Assumed Name for "The Party Porch" under existing LLC (TX SOS Form 503, ~$25)
- [ ] Dedicate existing bank account to the biz (clean books)
- [ ] 💰 Order the ~$600 starter kit (blasters + darts first)
- [ ] Design logo + brand kit in Canva
- [ ] Build 1-page site w/ booking form
- [ ] Set up booking intake: Google Form → Calendar
- [ ] Assemble + test kit; count darts, label bins
- [ ] Write safety rules + liability waiver
- [ ] 💰 Get event / general liability insurance quote + bind coverage (before first paid party)
- [ ] 📹 Film brand-launch content
- [ ] Fully set up all social profiles
- [ ] 📹 Run FREE pilot party w/ friends' kids
- [ ] 📣 Collect first testimonial + Google review
- [ ] 🎯 Post first 3 Reels/TikToks — brand LIVE

## August 2026 — Build + Soft Launch
- [ ] 📣 Announce "Now booking Fall parties!"
- [ ] 📣 Post in 10+ local Facebook groups
- [ ] List on Nextdoor + Marketplace
- [ ] 📣 Launch Founding Family offer: first 5 parties get the referee free
- [ ] 💰 Buy popcorn machine (~$200) → The Snack Bar
- [ ] Create pricing one-pager + booking link
- [ ] 📣 DM 3 local micro-influencer moms
- [ ] 📹 Run 2–3 soft-launch parties
- [ ] Refine run-of-show
- [ ] 📣 Contact 5 venues/parks re: recommended vendor
- [ ] Open September weekend slots
- [ ] 🎯 3+ parties done, 10+ content pieces, 5+ reviews

## September 2026 — 🚀 PUBLIC LAUNCH
- [ ] 📹 Official launch post + results reel
- [ ] 📣 Run launch giveaway
- [ ] 💰 Boosted ad ($5–10/day) on best video
- [ ] 🎯 Land first FULL-PRICE booking
- [ ] Hit 4–6 paid parties
- [ ] 📹 Launch Halloween "Zombie Survival" theme
- [ ] 📣 Ask every customer for review + footage
- [ ] 🎯 Phase 0 complete

## Phase 1 — Validate (Oct–Dec 2026)
- [ ] Run 4–8 parties/mo solo
- [ ] 📹 Post 4–5 videos/week
- [ ] Add Snack Bar to every booking
- [ ] 💰 Confirm insurance covers hired helpers/subs before Phase 2 hire
- [ ] Set up Stripe/Square deposits
- [ ] 📣 Holiday break camps + indoor venues
- [ ] 🎯 6+ bookings/mo, $500+ for Kit #2

## Phase 2 — Add-ons + First Hire (Jan–May 2027)
- [ ] 💰 Kit #2 · The Booth · The Backdrop · cotton candy
- [ ] Hire first part-time helper
- [ ] 📣 Weekday channels: schools, camps, churches
- [ ] 🎯 15+ parties/mo, avg ticket $380+

## Phase 3 — Multiply Crews (Jun–Oct 2027)
- [ ] 💰 Kit #3 · snow cone · Tiny Porch · 360 booth
- [ ] Launch Party Pack bundle
- [ ] 📣 Corporate/community events
- [ ] 🎯 25+ parties/mo, 2 crews

## Phase 4 — Hit +$10k/mo (Nov 2027–Mar 2028)
- [ ] Scale to 34+ parties/mo
- [ ] 💰 Glow Zone + movie nights
- [ ] Promote crew chief
- [ ] Sell Ultimate bundles
- [ ] 📣 Preferred-vendor deals
- [ ] 🏆 $10k+/mo NET — runs without you on-site
